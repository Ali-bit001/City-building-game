...
export function createCamera(gameWindow){
    ...
    function onMouseMove(event){
        ...
        if(isRightMouseDown){
            ...
            cameraRadius = Math.min(MAX_CAMERA_RADIUS,Math.max(MIN_CAMERA_RADIUS,camerarRadius));
            updateCameraPosition();
        }
    ...
    function updateCameraPosition(){
        camera.position.x = camerarRadius * Math.sin(cameraAzimuth * Math.PI/180) * Math.cos(cameraElevation * Math.PI/180);
        camera.position.y = camerarRadius * Math.sin(cameraElevation*Math.PI/180);
        camera.position.z = camerarRadius * Math.cos(cameraAzimuth * Math.PI/180) * Math.cos(cameraElevation * Math.PI/180);
